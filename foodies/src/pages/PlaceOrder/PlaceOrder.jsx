...
const PlaceOrder = () => {
  ...
  const onSubmitHandler = async (event) => {
    ...
    try {
      ...
      if (response.razorpayOrderId && response.key) {
        ...
        initiateRazorpayPayment(response.key, response);
      } else {
  ...
  const initiateRazorpayPayment = (key, order) => {
    const options = {
      key: key, // backend-provided public key
      ...
      theme: { color: "#3399cc" },
      modal: {
        ondismiss: () => deleteOrderHandler(order.razorpayOrderId),
      },
    };
